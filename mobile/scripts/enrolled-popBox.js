	...
	var popBox = {
		...
		submitInfo: function() { //报名提交验证
			...
			var submitFn = function() {
				...
				if (!reg.name.test(vName)) {
					...
					// http://www.940.com/enroll?user_name=1233&phone_number=15120332266&qq=454599069&do=add
					// $.ajax({
					// 	url: 'http://www.940.com/enroll?user_name=' + vName + '&phone_number=' + vTel + '&qq=' + vQQ + '&do=add',
					// 	type: 'POST',
					// 	async: false,
					// 	cache: false,
					// 	dataType: 'jsonp',
		...
		roll: function() {
			...
			function comSoll() {
				...
				_this.j_touch.stop().animate({
					top: (_window.height() - _this.j_touch.height()) / 2 + _top
				});
				_this.j_sign.stop().animate({
					top: (_window.height() - _this.j_touch.height()) / 2 + _top
				});
