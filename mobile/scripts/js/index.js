...
/*				Fengs.use('940/utils/slide');
			 ...
			 var login = {
				...
				var isLogin = {
			 	  ...
			 	  topLoding : function(){
			 	  	 var isiEText = function(){
				 	  	 	var iN = Number($.browser.version) , aIp = $('#revisePass').find('li');
				 	  	    if (iN>9) return;
							...
							aIp.each(function(i,obj){
								...
								if (i==0) {
									...
									$(this).append("<i class='prompt'>手机收到的验证码</i>");
									login.textChange(child);
								}else if(i==3){
									$(this).append("<i class='prompt'>新的登录密码</i>");
									login.passwordChange(child);
								}else if(i==4){
									$(this).append("<i class='prompt'>重复登录密码</i>");
									login.passwordChange(child);
								}
								
							})	
			 	  	 }
			 	  	 var revisePass = function(){
			 	  	 		layer.closeAll();
			 	  	 		layer.open({
						        type: 1,
						        title:'修改密码',
						        ...
						        moveType: 1,
						        content: '<div style="padding:0px 20px;margin: 0 auto;width: 240px;"><ul class="log_ul" id="revisePass">\
											<li class="txt_input"><input class="accounts"  type="text" placeholder="注册帐号 (邮箱)"></li>\
											<li class="but_submit"><input class="getCode" type="submit" value="点此获取手机验证码"></li>\
											<li class="txt_input"><input class="iphoneCode" type="text" placeholder="手机收到的验证码"></li>\
											<li class="txt_input"><input class="newPass" type="password" placeholder="新的登录密码"></li>\
											<li class="txt_input"><input class="cfPass" type="password" placeholder="重复登录密码"></li>\
											<li class="but_submit"><input class="revisePassBut" type="submit" value="提 交"></li>\
											</ul></div>',
						        success:function(){
						        	isiEText();
									...
									$revisePassBut.click(function(){
										if (!emailReg.test($accounts.val())) {
											...
											layer.msg('请填写登录密码！', {time: 2000});
											$newPass.focus();
										}else if($cfPass.val()==''){
											layer.msg('请填写重复密码！', {time: 2000});
											$cfPass.focus();
											...
											$.ajax({
							                    type: "post",
							                    url: "/find",
							                    data: {username:$accounts.val(),para:$cfPass.val(),code:$iphoneCode.val()},
							                    dataType: "json",
							                    success: function (data) {
							                    	layer.closeAll('loading');
							                    	var d = data;
			 	  	 ...
			 	  	 $('#topLoing,#regLoin').on('click',function(){
						        ...
						        content: '<div style="padding:0px 20px;margin: 0 auto;width: 240px;"><ul class="log_ul">\
											...
											<li class="but_reg"><a class="but_revisePass" href="javascript:">忘记密码</a> <a class="reg" href="/reg">注册账户</a></li>\
											</ul></div>',
						        success:function(){
						        	login.off = true;
						        	login.login(1);
						        	// userlog.submitFn()
						        	$('.but_revisePass').click(revisePass);
						        },
						        cancel :function(){
						        	login.off = false;
						        }
