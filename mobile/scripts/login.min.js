...
$(".btnCheck").click(function(){$(".login-check").toggleClass("checked");});function d(){var f=$.param({code:$(".captcha").val()});$.ajax({async:false,type:"post",url:"/checkCode",dataType:"json",data:f,success:function(g){if(g.status==0){alert(g.tips);
}if(g.status==1){alert("验证码正确");}if(g.status==-1){alert("没有定义");}if(g.status==-9){alert("验证码错误");}},error:function(){alert("加载失败！");}});}var b=$(".register-form");
b.bind("submit",function(f){f.preventDefault();var g=$.param({user_name:$(".username").val(),password:$(".password").val(),captcha:$(".captcha").val()});
$.ajax({async:false,type:"get",url:"/register",dataType:"json",data:g,success:function(h){d();if(h.status==1){alert(h.tips);window.location.href="login.html";
...
$.ajax({async:false,type:"get",url:"http://www.940.com/login",dataType:"jsonp",data:g,jsonp:"callback",success:function(i){console.log(g);if(i.status==1){alert(i.tips);
var h={u:$(".login-user").val(),password:$(".login-passw").val()};localStorage.member=JSON.stringify(h);window.location.href="user.html";}else{if(i.status==-9){alert(i.tips);
}else{alert("data.tips");}}}});});});